...
void	init_textures(t_env *e)
{
	...
	while (++i < NB_TEX)
		if (!(TEX[i] = malloc(sizeof(t_tex))))
			exit(EXIT_FAILURE);
	TX_IM(0) = mlx_xpm_file_to_image(MLX, "Img/wall6.xpm", &TXW(0), &TXW(0));
	...
	TX_IM(5) = mlx_xpm_file_to_image(MLX, "Img/floor4.xpm", &TXW(5), &TXW(5));
	TX_IM(6) = mlx_xpm_file_to_image(MLX, "Img/top1.xpm", &TXW(6), &TXW(6));
	TX_IM(7) = mlx_xpm_file_to_image(MLX, "Img/floor3.xpm", &TXW(7), &TXW(7));
